...
const PwSearch = (props) => {
  ...
  const verifiedHandle = () => {
    ...
    // 서버로 인증코드 요청
    Axios.post("http://118.67.134.86:3001/api/post/send_verification_code", {
      id: id,
      phone: totalPhone,
    })
      .then((res) => {
  ...
  const confirmHandle = () => {
    // 서버로 인증코드 확인 요청
    Axios.post("http://118.67.134.86:3001/api/post/verify_code_check", {
  ...
  const handleSubmit = () => {
    ...
    // 서버로 비밀번호 찾기 요청
    Axios.post("http://118.67.134.86:3001/api/post/pw_change", {
      id: id,
      phone: totalPhone,
    })
      .then((res) => {
